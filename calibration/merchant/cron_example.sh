#!/bin/bash
#
# Example cron script for scheduled merchant confidence calibration retraining
#
...
#   0 2 * * 0 /path/to/verireceipt/calibration/merchant/cron_example.sh
#
# Bi-weekly retraining (1st and 15th at 3 AM):
#   0 3 1,15 * * /path/to/verireceipt/calibration/merchant/cron_example.sh
#
# Monthly retraining (1st of month at 4 AM):
#   0 4 1 * * /path/to/verireceipt/calibration/merchant/cron_example.sh
#
...
DATA_DIR="${PROJECT_ROOT}/calibration/merchant/datasets"
ARTIFACTS_DIR="${PROJECT_ROOT}/calibration/merchant/artifacts"
...
echo "=========================================="
echo "Merchant Confidence Calibration Retraining"
...
LATEST_DATASET=$(find "${DATA_DIR}" -name "merchant_dataset_*.csv" -mtime -14 -type f | sort -r | head -1)

if [ -z "${LATEST_DATASET}" ]; then
    echo "❌ ERROR: No labeled dataset found in last 14 days"
    echo "   Expected files: ${DATA_DIR}/merchant_dataset_YYYYMMDD.csv"
    exit 1
fi

echo "📊 Using dataset: ${LATEST_DATASET}"
echo "   Size: $(du -h "${LATEST_DATASET}" | cut -f1)"
echo "   Modified: $(stat -c %y "${LATEST_DATASET}")"
...
if [ -f "${REPORT_FILE}" ]; then
    if grep -q "❌.*Regression Detected" "${REPORT_FILE}"; then
        ...
        #   -d "message=Calibration regression detected in ${LATEST_VERSION}"
        ...
        # python calibration/merchant/promote.py --version "${LATEST_VERSION}" --auto
...
find "${LOG_DIR}" -name "retrain_*.log" -mtime +${RETENTION_DAYS} -delete

# Cleanup old datasets (keep last 30 days)
echo "🧹 Cleaning up old datasets (older than 30 days)..."
find "${DATA_DIR}" -name "merchant_dataset_*.csv" -mtime +30 -delete
