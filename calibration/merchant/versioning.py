"""
Calibration artifact versioning utilities.

Provides deterministic version resolution and management for calibration artifacts.
"""
...
def parse_version(version: str) -> Tuple[int, str]:
    """Parse version string like 'merchant_v1_20260125' into (number, date)."""
    match = re.match(r'merchant_v(\d+)(?:_(\d{8}))?', version)
    if not match:
...
def generate_next_version(artifact_dir: Path, entity: str = "merchant") -> str:
...
def get_latest_version(artifact_dir: Path, entity: str = "merchant") -> Optional[str]:
...
def load_previous_artifact(artifact_dir: Path, entity: str = "merchant") -> Optional[dict]:
...
def get_artifact_path(artifact_dir: Path, version: str, entity: str = "merchant") -> Path:
    """Get the full path for an artifact file."""
    return artifact_dir / f"calibration_{version}.json"


def get_metrics_path(artifact_dir: Path, version: str, entity: str = "merchant") -> Path:
    """Get the full path for a metrics file."""
    return artifact_dir / f"metrics_{version}.json"


def get_report_path(artifact_dir: Path, version: str, entity: str = "merchant") -> Path:
    """Get the full path for a markdown report file."""
    return artifact_dir / f"calibration_report_{version}.md"


def get_summary_csv_path(artifact_dir: Path, entity: str = "merchant") -> Path:
    """Get the full path for the summary CSV file."""
    return artifact_dir / "calibration_summary.csv"


def get_bucket_csv_path(artifact_dir: Path, version: str, entity: str = "merchant") -> Path:
